  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5173/teacher-schedule/${teacherId}`);
        if (!response.ok) {
        ...
        // Fetch teacher info
        if (data.length > 0 && data[0].teacherInfo) {
          setTeacherInfo(data[0].teacherInfo);
        }
      } catch (error) {
    ...
    allEvents.forEach(event => {
      ...
      if (startIndex !== -1 && endIndex !== -1) {
        ...
        if (dayIndex !== -1) {
          ...
          const topOffset = startIndex * 24 - 12; // Offset to center on time line
          ...
          const eventData = {
            ...
            duration,
            heightInPixels,
            topOffset: Math.max(0, topOffset) // Prevent negative offset
          };
