...
function CoursesPage() {
  ...
  const fetchOfferingsForCourse = async (courseId) => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/offerings?${params}`); // CHANGED endpoint
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
  ...
  const deleteCourse = async (course) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/courses/${course._id}`, {
        method: "DELETE"
      });
