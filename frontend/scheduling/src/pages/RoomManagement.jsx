...
function RoomManagement() {
  ...
  const toggleRoomStatus = async (room) => {
    ...
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/api/rooms/${room._id}`, { isActive: !room.isActive });
  ...
  const handleSave = async (form) => {
    try {
      if (editingRoom) {
        await axios.put(`${import.meta.env.VITE_API_URL}/api/rooms/${editingRoom._id}`, form);
        Swal.fire({ icon: 'success', title: 'Updated', timer: 1200, showConfirmButton: false });
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL}/api/rooms`, form);
