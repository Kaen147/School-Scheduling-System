...
function EditSubjectModal({
      ...
      const workloadPromises = teachers.map(async (teacher) => {
        try {
          const response = await axios.get(
            `${import.meta.env.VITE_API_URL}/api/workload/teacher/${teacher._id}?academicYear=${formData.academicYear}&semester=${formData.semester}`
          );
  ...
  const updateTeacher = async (index, field, value) => {
    if (field === "teacherId" && value) {
      ...
      try {
        const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/workload/validate-assignment`, {
          teacherId: value,
        ...
        if (!response.data.success) {
          ...
          if (response.data.requiresOverload) {
            ...
            if (result.isConfirmed) {
              ...
              try {
                const overloadResponse = await axios.post(`${import.meta.env.VITE_API_URL}/api/workload/teacher/${value}/overload`, {
                  isOverloaded: true
                });
  ...
  const handleUpdate = async () => {
    try {
      ...
      for (const teacher of formData.assignedTeachers) {
        if (teacher.teacherId) {
          try {
            ...
            const validation = await axios.post(`${import.meta.env.VITE_API_URL}/api/workload/validate-assignment`, {
              teacherId: teacher.teacherId,
              subjectId: subject.subjectId._id,
              academicYear: formData.academicYear,
              semester: formData.semester,
            ...
            if (!validation.data.success) {
              ...
              if (validation.data.requiresOverload) {
                ...
                if (result.isConfirmed) {
                  // User confirmed overload - set teacher as overloaded
                  try {
                    const overloadResponse = await axios.post(`${import.meta.env.VITE_API_URL}/api/workload/teacher/${teacher.teacherId}/overload`, {
                      isOverloaded: true
                    });
      ...
      await axios.patch(
        `${import.meta.env.VITE_API_URL}/api/offerings/${subject._id}`,
        offeringPayload
      );
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        setLoading(true);
        await axios.delete(`${import.meta.env.VITE_API_URL}/api/subjects/${subject._id}`);
